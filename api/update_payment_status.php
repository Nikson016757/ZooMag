...
$stmt = $conn->prepare("UPDATE orders SET is_paid = ? WHERE id = ? AND courier_id = ?");
$stmt->bind_param("iii", $is_paid, $order_id, $courier_id);
if ($stmt->execute()) {
    redirect('/courier/index.php?success=payment_updated');
} else {
